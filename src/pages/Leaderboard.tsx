...
import { Trophy, Flame, Zap, Target, Crown, TrendingUp, Users, Sparkles, Medal } from 'lucide-react';

const tabs = [
  { label: 'Week', icon: TrendingUp, sortKey: 'xp' },
  { label: 'All Time', icon: Trophy, sortKey: 'xp' },
  { label: 'Streak', icon: Flame, sortKey: 'streak' },
  { label: 'Challenges', icon: Target, sortKey: 'challenges_completed' },
];
...
export default function Leaderboard() {
  ...
  const sortKey = tabs[tab].sortKey as keyof LeaderboardEntry;
  const sorted = [...allEntries].sort((a, b) => (b[sortKey] as number) - (a[sortKey] as number));

  let userRank = sorted.findIndex(e => (state.user.xp) > (e[sortKey] as number));
  if (userRank === -1) userRank = sorted.length;
  sorted.splice(userRank, 0, userEntry);

  const totalLearners = 847 + sorted.length;
  ...
  return (
    <AppLayout>
      <div className="max-w-2xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
          ...
          <div className="relative flex items-center gap-3 sm:gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-xs sm:text-sm flex items-center gap-1">
                <Users className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> {totalLearners.toLocaleString()} learners
              </p>
            </div>
          </div>
          ...
          {tabs.map((t, i) => (
            <motion.button
              ...
              className={`flex-1 py-2 sm:py-2.5 text-[10px] sm:text-xs font-medium rounded-lg transition-all flex items-center justify-center gap-1 sm:gap-1.5 ${
                tab === i ? 'bg-card text-foreground shadow-sm border border-border/50' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
        ...
        {/* Top 3 podium */}
        {sorted.length >= 3 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-3 gap-1.5 sm:gap-2">
            {[1, 0, 2].map((idx) => {
                  ...
                  <div className="relative">
                    ...
                    <p className="text-xs sm:text-sm font-heading font-bold text-foreground mt-0.5 sm:mt-1">
                      {sortKey === 'streak' ? `${entry.streak}🔥` : 
                       sortKey === 'challenges_completed' ? `${entry.challenges_completed} ✅` :
        ...
        <div className="space-y-1 sm:space-y-1.5">
          {loading ? (
            ...
            sorted.slice(3, 25).map((entry, i) => {
              const rank = i + 4;
              const maxXP = topEntry ? (topEntry[sortKey] as number) : 1;
              const value = entry[sortKey] as number;
              const barPct = Math.max(5, (value / maxXP) * 100);
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      ...
                      {entry.isUser && (
                        <span className="text-[9px] sm:text-[10px] px-1 sm:px-1.5 py-0.5 rounded-full bg-primary/20 text-primary font-semibold shrink-0">You</span>
                      )}
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs sm:text-sm font-heading font-bold text-foreground">
                      {sortKey === 'streak' ? (
                        <span className="flex items-center gap-1"><Flame className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-secondary" />{entry.streak}d</span>
                      ) : sortKey === 'challenges_completed' ? (
                        <span className="flex items-center gap-1"><Target className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-accent" />{entry.challenges_completed}</span>
                      ) : (
                        <span className="flex items-center gap-1"><Zap className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-secondary" />{value.toLocaleString()}</span>
                      )}
                    </p>
                  </div>
                </motion.div>
